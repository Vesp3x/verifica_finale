...
* **Eliminazione dei valori nulli:** Per la gestione dei dati mancanti si è deciso di procedere con due approcci differenti in base all'importanza della caratteristica:
  * *La Marca dell'Auto:* Se in un annuncio manca il nome del produttore (ad esempio se non sappiamo se l'auto sia una Ford o una Fiat), quella riga viene completamente eliminata. L'operazione si compone di due passaggi logici concatenati: la rimozione del record incompleto dalle caratteristiche del veicolo e il successivo riallineamento con la tabella dei prezzi, operazione fondamentale per evitare sbilanciamenti che manderebbero il modello in crash.
